...
from src.prizeNodes import P_Node, P_PrizeNodes


from src.graphClass import Graph
from data.RomaniaMapData import mazeData
...
def buildGraph(graphData, nodeColorsDict):
    ...
    netRomania.save_graph('maze.html')
...
def main(): 
    ...
    if not st.session_state["clicked"]:
        # Set header title
        st.header("Problem Solving Agents: Maze Problem")
        st.header("_Initial Env._", divider=True)
        
        romaniaGraph = Graph(mazeData)
        romaniaGraph.gen_prize_nodes(mazeData)
        
        nodeColors=makeDefaultColors(romaniaGraph.graph_dict)
        
        initState="N1"
        goalState = [list(node for node in P_PrizeNodes.nodes_w_prize), "N32"]
        
        # DEBUG
        # print all nodes with prizes
        for node in P_PrizeNodes.nodes_w_prize:
            print(f"{node.node_id} -> Prize = {node.prize}")
        ...
        #nodeColors[BFSnavAgent.goal[0]]="green"
